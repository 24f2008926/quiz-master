...
class QZ_ATM_S(db.Model):
    QZ_ATS_ATMT_ID = db.Column(db.String(10), db.ForeignKey('qz_atm_hdr.QZ_ATM_ATMT_ID'), primary_key=True)  
    QZ_ATS_DATE = db.Column(db.Date)  
    QZ_ATS_TTKN = db.Column(db.Time)  
    QZ_ATS_SCORE = db.Column(db.Integer)  
    QZ_ATS_CO = db.Column(db.Integer) 
    QZ_ATS_INCO = db.Column(db.Integer)  


    attempt = db.relationship('QZ_ATM_HDR', backref='summary')  
